...
export default async function handler(req, res) {
  ...
  if (!OPENAI_API_KEY) return res.status(500).json({ error: 'API key no configurada' });
  ...
  try {
    ...
    if (type === 'image' && imageBase64) {
      ...
      messages = [{ role: 'user', content: `Sos un asistente academico. Resume el siguiente texto de forma clara y estructurada en español. Incluí: puntos clave, ideas principales y conceptos importantes.\n\n${text}` }];
    }

    const openaiResp = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${OPENAI_API_KEY}` },
      body: JSON.stringify({ model: type === 'image' ? 'gpt-4o' : 'gpt-4o-mini', messages, max_tokens: 1500, temperature: 0.3 })
    });
